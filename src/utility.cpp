...
bool triangle(int l1, int l2, int L){
  bool ret = true;
  if (L < abs(l1-l2)) ret = false;
  if (L > l1+l2) ret = false;
  return ret;
}
...
string extract_string(string input_string, string var_name, string default_value){
  ...
  {
    offset = s.size() + found;
    flag = extract_stringNew(result, input_string, offset);
...
vector<double> extract_vector_double(string input_string, string var_name, vector<double> default_value){
  ...
  if (!quiet){
    cout << "Extracting vector<double>, varname = " << var_name << endl;
    cout << "default_value = "; print_vector(default_value);
  }
...
bool read_array_from_file(vector<double> &tar, string file_name, string identifier, int desiredArraySize){
  tar = vector<double>();
  ifstream inFile;
