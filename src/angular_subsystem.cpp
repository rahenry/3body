...
angular_subsystem::angular_subsystem(controller *cont_) :
  name(cont_->name), cont(cont_), ltotal(cont_->ltotal), lmax_r(cont_->lmax_r), lmax_rho(cont_->lmax_rho), lmax_coup(cont_->lmax_coup), lmax_com(cont_->lmax_com), single_angular_state(cont_->single_angular_state), parity(cont_->parity)
{
  lmax = max(
...
void angular_subsystem::generate_E(){
  ...
  for (int l9=0; l9<lim1; l9++){
    ...
    res *= sqrt((2.0*l3+1.0)*(2.0*l6+1.0)*(2.0*l7+1.0)*(2.0*l8+1.0));
...
void angular_subsystem::generate_Q(){
  ...
  for (auto &s1 : states){
    for (auto &s2 : states_transformed){
      for (int a=0; a<lim_special; a++){
	for (int b=0; b<lim_special; b++){
	  for (int mcom=-lmax; mcom<=lmax; mcom++){
	    double res = 0.0;
...
void angular_subsystem::generate_F(){
  ...
  for (auto &s1 : states){
	...
	if (s1.lcom == s2.lcom){
          for (int a=0; a<lim_special; a++){
            for (int b=0; b<lim_special; b++){
              double E1 = E(kappa,kappa,0,s1.lr,s1.lrho,s1.lcoup,a,b,s1.lcoup);
              for (int mcom=-s2.lcom; mcom<=s2.lcom; mcom++){
...
void angular_subsystem::print_angular_states(){


  cout << "Angular states: " << endl;
  for (auto &a : states){
    cout << a << endl;
  }
  cout << "Transformed angular states: " << endl;
  for (auto &t : states_transformed){
    cout << t << endl;
  }
}

void angular_subsystem::generate_generation_modes(){
  cout << "Determining generation modes... " << endl;
  generation_modes = vector< vector<int> >();
  for (int mode = 0; mode < cont->max_states.size(); mode++){
    generation_modes.push_back(vector<int>());

    for (auto & s : states)
      if (generation_mode_test(s, mode)) 
        generation_modes.back().push_back(s.index);

    if (generation_modes.back().size() == 0 && mode == 0){
      cout << "Mode 0 generation failed, reassigning to mode 1..." << endl;
      generation_modes = vector< vector<int> >();
      continue;
    }

    cout << "Mode " << mode << ":" << endl;
    print_vector(generation_modes.back());
    
  }
}


bool angular_subsystem::generation_mode_test(state_angular &s, int mode){
  int test = max(0, s.lr);
  test = max(test, s.lrho);
  test = max(test, s.lcoup);
  test = max(test, s.lcom-1);
